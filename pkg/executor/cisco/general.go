...
type CiscoProfileGeneral struct {
	*CiscoProfileBase
}

func NewCiscoProfileGeneral(promptWithRegex bool, prompt, enaPrompt, enaPass string) *CiscoProfileGeneral {
	return &CiscoProfileGeneral{
		CiscoProfileBase: newCiscoProfileBase(promptWithRegex, prompt, enaPrompt, enaPass),
	}
}

func (p *CiscoProfileGeneral) GetType() string { return CiscoProfileTypeGeneral }

func (p *CiscoProfileGeneral) GetTask(nodeName string,
...
func (p *CiscoProfileGeneral) Login(nodeName string, emitter core.SshCExecutorEmitter,
	...
	if opts.WithEna {
		...
		_, err = session.CstdinPipe.Write([]byte("ena" + "\r\n"))
		if err != nil {
		...
		if n > 2 && buff[0] == '\r' && buff[1] == '\n' {
			return output, fmt.Errorf("unexpected state on manage ena (%s)", string(buff[0:n]))
		}
